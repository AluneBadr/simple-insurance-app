...
#' 
#' @param value_date date the valuation date of the loss run.  Claim values from `trans`
#' will be values as of the `value_date`
#' @param trans data frame of claims transactions
#' 
#' @importFrom dplyr `%>%` filter group_by top_n ungroup mutate arrange
#' 
#' @return data frame of claims (1 claim per row) valued as of the `value_date`
#' 
loss_run <- function(value_date, trans_ = trans) {
  trans_ %>%
    filter(transaction_date <= value_date) %>%
    group_by(claim_num) %>%
