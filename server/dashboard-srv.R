...
ay_plot_prep <- reactive({
  ...
  titles <- switch(
    dash_metric_input,
    "total" = list(
      "title" = paste0("Reported Loss & ALAE as of ", value_date),
      "subtitle" = subtitle,
      "series" = c("Paid", "Case Reserve"),
      "y_axis" = "Loss & ALAE"
    ),
    "severity" = list(
      "title" = paste0("Reported Severity as of ", value_date),
      "subtitle" = subtitle,
      "series" = c("Paid Severity", "Case Reserve Severity"),
      "y_axis" = "Loss & ALAE"
    ),
    "claims" = list(
      "title" = paste0("Reported Claims as of ", value_date),
...
output$ay_plot <- renderHighchart({
  ...
  highchart() %>%
    ...
    hc_add_series(
      ...
      name = titles$series[2], color = "#F5B7B1"
    ) %>%
    hc_add_series(
      data = round(dat$paid, 0),
      name = titles$series[1], color = "#82E0AA"
    ) 
})


output$title <- renderText({paste0("Predictions of the year ", lubridate::year(input$value_date))})


output$gauge_paid <- flexdashboard::renderGauge({
  Pred <- val_tbl() %>%
    #mutate(month = lubridate::month(trans$accident_date, label = T))%>%
    mutate(year = lubridate::year(accident_date))%>%
    filter(year < input$value_date)%>%
    group_by(year) %>%
    summarise(paid = sum(paid),
              case = sum(case),
              n = n())%>%
  ...
  gauge(mean(round(Pred$paid)), min = 0, max = round(max(Pred$paid)), symbol = '$', label = paste("Paid & ALAE")
        ,gaugeSectors(colors = c("#7B241C"))
        )

})

output$gauge_case <- flexdashboard::renderGauge({
  Pred <- val_tbl() %>%
    #mutate(month = lubridate::month(trans$accident_date, label = T))%>%
    mutate(year = lubridate::year(accident_date))%>%
    filter(year < input$value_date)%>%
  ...
  gauge(mean(round(Pred$case)), min = 0, max = round(max(Pred$case)), symbol = '$', label = paste("Case Reserve")
        ,gaugeSectors( colors = c("#2874A6"))
  )
  
})

output$gauge_claims <- flexdashboard::renderGauge({
  Pred <- val_tbl() %>%
    #mutate(month = lubridate::month(trans$accident_date, label = T))%>%
    mutate(year = lubridate::year(accident_date))%>%
    filter(year < input$value_date)%>%
  ...
  gauge(round(mean(Pred$n)), min = 0, max = round(max(Pred$n)), symbol = '', label = paste("Claims")
        ,gaugeSectors(colors = c("#F1C40F"))
  )
